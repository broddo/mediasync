...
class MediaBrowser():
    ...
    def get_user_items(self, id, type, played=False):
        ...
        if response.status_code != 200:
            raise APIError(self.name, response, f"Failed to retrieve items for user id '{id}'")
    ...
    def set_watched(self, user_id, items):
        for item in items:
            ...
            if response.status_code != 200:
                raise APIError(self.name, response, f"Failed to update watch status for item {item['Id']}, user '{user_id}'")
...
def CreateMediaServer(type, url, key):
    ...
    try:
        ...
        raise SystemExit(f'Unknown server type {k}. Only Emby and Jellyfin are supported.')
