...
def load_settings(path):
    with path.open('r') as file:
        ...
        if 'source' not in settings or 'destination' not in settings:
            raise ValueError(f"Both a source and desintation object must be defined in {path}")
    return settings
...
def sync_items(src, dst, user_id, type):
    ...
    migration = [item for item in dest_items if 'Path' in item and not item['UserData']['Played'] and search(item, src_items)]
...
def diff(options):
    ...
    src_items = src.get_items('Movie')
    dest_items = dst.get_items('Movie')
    migration = [item for item in src_items if 'Path' in item and not search(item, dest_items)]
    if (len(migration)):
        print(f"The following {len(migration)} movies from source are missing from destination")
        for item in migration:
            print(f"ID: '{item['Id']}', '{item['Name']}', path: '{item['Path']}'")
    else:
        print(f"All movies in source were matched with movies in destination")

    src_items = src.get_items('Episode')
    dest_items = dst.get_items('Episode')
    migration = [item for item in src_items if 'Path' in item and not search(item, dest_items)]
    if (len(migration)):
        print(f"The following {len(migration)} episodes from source are missing from destination")
        for item in migration:
            print(f"ID: '{item['Id']}', '{item['Name']}', '{item['SeasonName']}, series: '{item['SeriesName']}', path: '{item['Path']}'")
    else:
        print(f"All episodes in source were matched with episodes in destination")
...
def main():
    ...
    parser_diff = subparsers.add_parser("diff", help="Report differences between both libraries and exit")
    parser_diff.set_defaults(func=diff)

    parser_sync = subparsers.add_parser("sync", help="Synchronize the watched status of source with the destination")
    parser_sync.set_defaults(func=sync)
    parser_sync.add_argument("-p", "--password", default=None, help="Set default user password used during account creation. Omit to use random password")
